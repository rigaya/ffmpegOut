#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""ffmpegOut 終了バッチ用: 出力ファイル / ログ / aup2 設定のスモークチェック。"""
...
def extract_log_ffmpeg_options(log_text: str) -> str | None:
    lines = log_text.splitlines()
    for idx, line in enumerate(lines):
        if "ffmpeg options" in line:
            buf: list[str] = []
            for fol in lines[idx + 1 :]:
                ...
                if not s:
                    ...
                    continue
                # 次セクションっぽい行で打ち切り
                if re.match(r"^(AAC|raw |ffmpeg \[|L-SMASH|CPU|encoded |converting |Aviutl)", s):
                    break
                if s.startswith("---"):
                    break
                buf.append(s.strip())
...
def compare_cli(expected: str, actual: str) -> list[str]:
    ...
    if not expected:
        return problems
    if not actual:
        problems.append("ログから ffmpeg options を抽出できませんでした")
        return problems
...
def main() -> int:
    ap = argparse.ArgumentParser(description="ffmpegOut smoke check (after-bat)")
    ...
    name_keys = cfg.get("plugin_name_contains") or ["ffmpegOut"]
    ...
    if not logpath.is_file():
        ...
        log_cli = extract_log_ffmpeg_options(log_text)
        if log_cli:
            ok(f"ログ CLI 抽出: {log_cli[:120]}{'...' if len(log_cli) > 120 else ''}")
        else:
            ng("ログから ffmpeg options を抽出できませんでした")
        ...
        if aup2 and aup2.is_file():
            try:
                ...
                if conf is None:
                    ng(f"aup2 から ffmpegOut 設定を見つけられません: {aup2}")
                else:
