...
<#
.SYNOPSIS
  ffmpegOut をビルドし、AviUtl2 Plugin へ配置して起動する。
...
$Sln = Join-Path $RepoRoot "ffmpegOut.sln"
...
if ($Platform -eq "x64") {
    $srcAuo2 = Join-Path $RepoRoot "$Configuration\x64\ffmpegOut.auo2"
} else {
    $srcAuo2 = Join-Path $RepoRoot "$Configuration\Win32\ffmpegOut.auo"
}
...
$dstAuo2 = Join-Path $PluginDir "ffmpegOut.auo2"
Write-Host "==> copy"
Write-Host "    $srcAuo2"
Write-Host " -> $dstAuo2"
...
foreach ($name in @("ffmpegOut.ini", "ffmpegOut.en.ini", "ffmpegOut.zh.ini")) {
